...
const { coversDir } = require('./issueCovers');
...
function syncArchiveIssueCovers(db) {
  ...
  folders.forEach((folder, index) => {
    ...
    const coverPublic = path.join(coversStorage, `${n}.png`);
    ...
    const existingRow = db.prepare('SELECT id, cover_image FROM issues WHERE archive_folder = ?').get(folder)
      || db.prepare('SELECT id, cover_image FROM issues WHERE title = ?').get(folder);
    const adminCover = existingRow?.cover_image
      && /^\/covers\/issues\/\d+\.[a-z0-9]+$/i.test(existingRow.cover_image);
    const coverPath = adminCover
      ? existingRow.cover_image
      : (fs.existsSync(coverPublic) ? `/covers/issues/${n}.png` : null);
