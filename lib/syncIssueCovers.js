...
const { parseStemFolderMeta } = require('./sortArchiveFolders');
...
/**
 * Синхронизирует выпуски STEM с папками архива и обложками cover.png / covers/issues/N.png
 */
function syncArchiveIssueCovers(db) {
  const archivesRoot = path.join(__dirname, '..', 'public', 'archives');
  ...
  const insert = db.prepare(`
    ...
    VALUES ('stem', @title, NULL, @sortOrder, @accepting, @issuedAt, @folder, @coverPath)
  `);
