...
const { normalizeAuthors } = require('./articleMetadata');
const { invalidateArchiveIndexCache } = require('./archiveSearch');

const UPLOADS_DIR = path.join(__dirname, '..', 'uploads');
const ARCHIVE_EXTS = new Set(['.doc', '.docx', '.pdf']);
...
async function publishSubmissionToArchive(submissionId, db) {
  ...
  if (!sub.file_path) return { ok: false, error: 'У статьи нет файла' };
  ...
  const srcPath = path.join(UPLOADS_DIR, path.basename(sub.file_path));
  if (!fsSync.existsSync(srcPath)) {
    return { ok: false, error: 'Исходный файл статьи не найден на сервере' };
  }
...
module.exports = { publishSubmissionToArchive, syncPublishedToArchive };
