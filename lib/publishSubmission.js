...
function safeArchiveFileName(name) {
  ...
  if (base !== name.trim() || base.includes('..')) return null;
  if (!ARCHIVE_EXTS.has(path.extname(base).toLowerCase())) return null;
  return base;
}

function resolveIssueArchiveFolder(db, issueId) {
  ...
  if (!folderRel) {
    return {
      ok: false,
      error: 'У выпуска не задана «Папка в архиве». Укажите её в разделе «Выпуски журналов».',
    };
  }

  const folderAbs = resolveArchivePath(archivesDirectory(), folderRel);
  if (!folderAbs) return { ok: false, error: 'Некорректный путь папки архива' };

  return { ok: true, issue, folderRel, folderAbs };
}
...
/**
 * Список PDF-статей в архиве выпуска.
 */
async function listIssueArchiveArticles(db, issueId) {
  const id = Number(issueId);
  if (!Number.isFinite(id)) return { ok: false, error: 'Некорректный выпуск' };

  const resolved = resolveIssueArchiveFolder(db, id);
  if (!resolved.ok) return resolved;

  const { issue, folderRel, folderAbs } = resolved;
  const filesMap = await readArticlesJson(folderAbs);
  let dirents = [];
  try {
  ...
  for (const entry of dirents) {
    ...
    if (!ARCHIVE_EXTS.has(path.extname(entry.name).toLowerCase())) continue;
    const meta = filesMap[entry.name] || {};
    articles.push({
      file: entry.name,
      title: meta.title || entry.name.replace(/\.pdf$/i, ''),
      authors: Array.isArray(meta.authors) ? meta.authors : normalizeAuthors(meta.authors),
      publishedAt: meta.publishedAt || null,
      quickPublish: !!meta.quickPublish,
      submissionId: meta.submissionId || null,
    });
  }

  articles.sort((a, b) => a.file.localeCompare(b.file, 'ru', { numeric: true }));
...
/**
 * Удаляет PDF из архива выпуска и запись в articles.json.
 */
async function removeArchiveArticle(db, options = {}) {
  const issueId = Number(options.issueId);
  const fileName = safeArchiveFileName(options.fileName);
  if (!Number.isFinite(issueId)) return { ok: false, error: 'Некорректный выпуск' };
  if (!fileName) return { ok: false, error: 'Некорректное имя PDF-файла' };

  const resolved = resolveIssueArchiveFolder(db, issueId);
  if (!resolved.ok) return resolved;

  const { folderRel, folderAbs } = resolved;
  ...
  await writeArticlesJson(folderAbs, filesMap);

  db.prepare(`
    UPDATE submissions
    SET published_archive_file = NULL
    WHERE issue_id = ? AND published_archive_file = ?
  `).run(issueId, fileName);
...
module.exports = {
  publishSubmissionToArchive,
  syncPublishedToArchive,
  publishPdfToIssueArchive,
  listIssueArchiveArticles,
  removeArchiveArticle,
};
